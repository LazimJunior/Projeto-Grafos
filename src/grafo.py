...
class EdgeItem(QGraphicsLineItem):
    """Representa uma aresta entre dois nós."""

    def __init__(self, source_node: NodeItem, dest_node: NodeItem):
        super().__init__()
        ...
        self.text_item_edge.setZValue(1)  # texto fica sobre a aresta
...
class GraphView(QGraphicsView):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            else:  # Criação de um novo nó (Manualmente)
                if self.selected_node:
                    ...
                    if label is None:
                        QMessageBox.warning(self, "Erro de Rótulo",
                                            "Não foi possível gerar um rótulo único (A-Z). Todos estão em uso.")
                        super().mousePressEvent(event)
                        return
